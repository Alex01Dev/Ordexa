...
export const ScannerScreen = ({ navigation, route }: Props) => {
  ...
  const handleScan = async ({ data: code }: { data: string }) => {
    ...
    if (mode === 'SALE_ITEM') {
      ...
      if (result === 'no-stock') {
        Alert.alert('Sin stock suficiente', `Ya no hay más unidades disponibles de "${existing.name}".`, [
          { text: 'Entendido', onPress: () => navigation.goBack() },
        ]);
        return;
      }
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.overlay}>
        ...
        <Text style={styles.hint}>{loading ? 'Procesando...' : 'Apunta al código de barras'}</Text>
      </View>
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  frame: { width: 250, height: 120, borderWidth: 2, borderColor: '#fff', borderRadius: 12, marginBottom: 16 },
